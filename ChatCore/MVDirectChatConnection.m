...
+ (id) directChatConnectionWithUser:(MVChatUser *) user passively:(BOOL) passive {
	...
	if( passive ) {
		...
		[user sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"CHAT chat 16843009 0 %lu", (unsigned long)passiveId]];
	} else {
		[ret initiate];
	}
...
- (void) directClientConnection:(MVDirectClientConnection *) connection acceptingConnectionsToHost:(NSString *) host port:(unsigned short) port {
	...
	if( [self isPassive] ) [[self user] sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"CHAT chat %@ %hu %lld", address, [self port], [self _passiveIdentifier]]];
