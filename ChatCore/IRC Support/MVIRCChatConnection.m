...
/*	NSString *stringWithPasswordsHidden = [string stringByReplacingOccurrencesOfRegex:@"(^PASS |^AUTHENTICATE (?!\\+$|PLAIN$)|IDENTIFY (?:[^ ]+ )?|(?:LOGIN|AUTH|JOIN) [^ ]+ )[^ ]+$" withString:@"$1********" options:NSRegularExpressionCaseInsensitive range:NSMakeRange(0, string.length) error:NULL]; */
    NSString *stringWithPasswordsHidden = string;
		...
		if( [command isCaseInsensitiveEqualToString:@"VERSION"] ) {
			...
			if( [subCommand isCaseInsensitiveEqualToString:@"SEND"] ) {
				...
				if( [address rangeOfCharacterFromSet:[NSCharacterSet characterSetWithCharactersInString:@".:"]].location == NSNotFound ) {
					...
					address = [NSString stringWithFormat:@"%u.%u.%u.%u", (ip4 & 0xff000000) >> 24, (ip4 & 0x00ff0000) >> 16, (ip4 & 0x0000ff00) >> 8, (ip4 & 0x000000ff)];
				}
