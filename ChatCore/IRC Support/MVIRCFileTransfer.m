...
+ (id) transferWithSourceFile:(NSString *) path toUser:(MVChatUser *) user passively:(BOOL) passive {
	...
	if( passive ) {
		...
		if( [ret _fileNameQuoted] ) [user sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"SEND \"%@\" 16843009 0 %llu %lu T", fileName, [ret finalSize], (unsigned long)passiveId]];
		else [user sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"SEND %@ 16843009 0 %llu %lu T", fileName, [ret finalSize], (unsigned long)passiveId]];
	} else {
		[ret _setupAndStart];
	}
...
- (void) reject {
	if( [self isPassive] ) {
		if( _fileNameQuoted ) [[self user] sendSubcodeReply:@"DCC" withArguments:[NSString stringWithFormat:@"REJECT SEND \"%@\" 16843009 0 %llu %lld T", [self originalFileName], [self finalSize], [self _passiveIdentifier]]];
		else [[self user] sendSubcodeReply:@"DCC" withArguments:[NSString stringWithFormat:@"REJECT SEND %@ 16843009 0 %llu %lld T", [self originalFileName], [self finalSize], [self _passiveIdentifier]]];
	} else {
		NSString *address = [self host];
		if( ! address ) address = @"16843009";
		if( address && [address rangeOfString:@"."].location != NSNotFound )
			address = [NSString stringWithFormat:@"%u", ntohl( inet_addr( [address UTF8String] ) )];
...
- (void) acceptByResumingIfPossible:(BOOL) resume {
	if( resume ) {
		...
		if( fileExists && [size unsignedLongLongValue] < [self finalSize] ) {
			if( [self isPassive] ) {
				if( _fileNameQuoted ) [[self user] sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"RESUME \"%@\" 0 %llu %lldd", [self originalFileName], [size unsignedLongLongValue], _passiveId]];
				else [[self user] sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"RESUME %@ 0 %llu %lld", [self originalFileName], [size unsignedLongLongValue], _passiveId]];
			} else {
...
- (void) directClientConnection:(MVDirectClientConnection *) connection acceptingConnectionsToHost:(NSString *) host port:(unsigned short) port {
	...
	if( _fileNameQuoted ) [[self user] sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"SEND \"%@\" %@ %hu %llu %lld", fileName, address, [self port], [self finalSize], [self _passiveIdentifier]]];
	else [[self user] sendSubcodeRequest:@"DCC" withArguments:[NSString stringWithFormat:@"SEND %@ %@ %hu %llu %lld", fileName, address, [self port], [self finalSize], [self _passiveIdentifier]]];
}
