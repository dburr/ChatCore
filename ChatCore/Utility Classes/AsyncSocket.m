...
- (BOOL)acceptOnAddress:(NSString *)hostaddr port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket4)
	{
		...
		//DLog(@"theSocket4: %hu", [self localPort:theSocket4]);
	}
...
- (NSString *)description
{
	...
	[ms appendString:[NSString stringWithFormat:@"has queued %u reads %u writes, ", (unsigned int)readQueueCount, (unsigned int)writeQueueCount]];
	...
	else
	{
		...
		[ms appendString: [NSString stringWithFormat:@"currently read %u bytes (%d%% done), ",
			...
			(unsigned int)theCurrentRead->bytesDone ? percentDone : 0]];
	}
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			DLog (@"AsyncSocket %p received unexpected CFSocketCallBackType %ld.", self, type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			DLog (@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %ld.", self, type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			DLog (@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %ld.", self, type);
	}
}
